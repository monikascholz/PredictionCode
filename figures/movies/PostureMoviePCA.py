...
# rescale avP because we didn't do so in the prediction code
avP = (avP *np.std(avTrue))+ np.mean(avTrue)
...
axscheme1.set_title('PCA model', y=1.0)
...
mp.make_animation3(fig, axscheme1,timeActual, axscheme2, cl, clPred+(500,0), frames=test, color = N0, save= 1, fname='crawlingPCA.mp4',velocity=velocity)
# animate wor
plt.show()