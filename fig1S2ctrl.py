...
#
################################################
fig = plt.figure('Fig1 - S2 : Neural dynamics in immobile transitions', figsize=(9.5, 9*2/4.))
gsHeatmap = gridspec.GridSpec(3,4,  width_ratios=[1, 1, 1, 0.1], height_ratios = [1,0.1,0.75])
gsHeatmap.update(left=0.055, right=0.99,  bottom = 0.15, top=0.95, hspace=0.1, wspace=0.45)
...
for key, axhm,axetho, axpc, dset, title  in zip([movData, immData, transientData], \
    [axhm1, axhm2, axhm3], [axetho1, axetho2, axetho3],[axpc1, axpc2, axpc3],\
    [data['AML18_moving'],data['AML18_immobilized'], data['Special_transition']],\
    ['moving', 'immobilized', 'transient']):
