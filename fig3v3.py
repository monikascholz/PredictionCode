...
gs1 = gridspec.GridSpec(3, 2, width_ratios = [0.5,0.5], height_ratios = [0.75,1,1])
gs1.update(left=0.15, right=0.96, wspace=0.45, bottom = 0.01, top=0.99, hspace=0.2)
...
# find minimal distances - this is the atlas ID. But ensure that assignments are unique
candNeurons = np.argmin(D, axis=1)
neuralID = []
for i in range(len(AB)):
    # pick up all close neurons
    relevantIDs = np.where(D[i]<5)
    # sort the labels from closer to farther
    relevantIDs=np.argsort(D[i][D[i]<5])
    neuralID.append(labels[relevantIDs])
neuralID = np.array(neuralID)

# replace best guess with unique assignment
bestGuess = np.concatenate(bestGuess)#labels[np.argmin(D, axis=1)]
